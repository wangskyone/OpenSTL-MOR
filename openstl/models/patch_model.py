...
from openstl.modules import (ConvSC, ConvNeXtSubBlock, ConvMixerSubBlock, GASubBlock, gInception_ST,
                             ...
                             SwinSubBlock, UniformerSubBlock, VANSubBlock, ViTSubBlock, TAUSubBlock,MoDViTSubBlock)


class Patch_Model(nn.Module):

    def __init__(self, in_shape, hid_S=32,hid_T=256, patch_size=2, N_T=4, model_type='gSTA',
                 mlp_ratio=8., drop=0.0, drop_path=0.0, act_inplace=True, **kwargs):
        super(Patch_Model, self).__init__()
        T, C, H, W = in_shape  # T is pre_seq_length
        H, W = int(H / patch_size), int(W / patch_size)  # downsample 1 / 2**(N_S/2)
        act_inplace = False
        self.num_patches_h = H 
        self.num_patches_w = W 
        self.patch_size = patch_size
        self.enc = nn.Conv2d(in_channels=C,out_channels=hid_S,kernel_size=patch_size,stride=patch_size) # patch 化
         # 学习型绝对位置编码（无 cls）
        self.pos_embed = nn.Parameter(torch.zeros(1, hid_S, self.num_patches_h, self.num_patches_w), requires_grad=True)
        nn.init.trunc_normal_(self.pos_embed, std=0.02)
        self.dec = nn.Sequential(nn.BatchNorm2d(hid_S), nn.Conv2d(hid_S, C * self.patch_size ** 2, 1)) 
    ...
    def forward(self, x_raw, **kwargs):
        ...
        embed = self.enc(x)
        embed = embed + self.pos_embed # 添加位置编码
        ...
        Y = self.dec(hid).reshape(B*T,C,self.patch_size,self.patch_size,H_,W_)
        Y = Y.permute(0, 1, 2, 4, 3, 5).reshape(B*T, C, H, W).reshape(B, T, C, H, W)
        return Y
...
class MetaBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, input_resolution=None, model_type=None,
                 mlp_ratio=8., drop=0.0, drop_path=0.0, layer_i=0,depth=6):
        ...
        if model_type == 'gsta':
            ...
            self.block = ViTSubBlock(
                in_channels, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path)
        elif model_type == 'modvit':
            self.block = MoDViTSubBlock(
                in_channels, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path, layer_i=layer_i,depth=depth)
        elif model_type == 'tau':
...
class MidMetaNet(nn.Module):
    ...
    def __init__(self, channel_in, channel_hid, N2,
        ...
        enc_layers = [MetaBlock(
            ...
            mlp_ratio, drop, drop_path=dpr[0], layer_i=0,depth=N2)]
        # middle layers
        for i in range(1, N2-1):
            enc_layers.append(MetaBlock(
                channel_hid, channel_hid, input_resolution, model_type,
                mlp_ratio, drop, drop_path=dpr[i], layer_i=i,depth=N2))
        # upsample
        enc_layers.append(MetaBlock(
            channel_hid, channel_in, input_resolution, model_type,
            mlp_ratio, drop, drop_path=drop_path, layer_i=N2-1,depth=N2))
        self.enc = nn.Sequential(*enc_layers)
